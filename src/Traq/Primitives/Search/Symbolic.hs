...
instance (Show sizeT) => SerializePrim (QSearchSym sizeT precT) where
  ...
  parsePrimParams tp "any_sym" = QAnySym <$> parsePrimParams tp "any"
  ...
  printPrimParams (QAnySym prim) = printPrimParams prim
...
instance (P.TypingReqs sizeT) => TypeCheckPrim (QSearchSym sizeT precT) (Sym.Sym sizeT) where
  ...
  inferRetTypesPrim (QAnySym p) = inferRetTypesPrim p
...
getSearchType (QSearchSym (PrimSearch ty)) = ty
getSearchType (QAnySym (PrimAny ty)) = ty
