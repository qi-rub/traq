...
spec = do
  describe "matrix search example" $ do
    ...
    it "unitary cost for delta=0.0001" $ do
      ...
      let cu = P.unitaryQueryCost qformulas delta ex
    ...
    it "quantum cost for eps=0.0001" $ do
      let eps = 0.0001
      let cq = P.quantumQueryCost qformulas eps ex oracleF Ctx.empty
    ...
    describe "lower to CQPL" $ do
      ...
      it "lowers" $ do
        assertRight $ CQPL.lowerProgram qalgo Ctx.empty eps ex
